...
def analyze_a_share_characteristics(context: MarketContext) -> SkillInsight:
    ...
    if missing or context.data_status != "verified":
        return SkillInsight(
            ...
            conclusion="涨停封板、炸板或一字板核心数据不完整，不能解释短线情绪结构。",
            strategy="补齐同一交易日的真实涨停与炸板池后重算，不把缺失值当作零。",
            evidence=[f"缺失字段：{', '.join(missing) if missing else '无'}", f"数据时间：{context.as_of}"],
            risks=list(context.unavailable_reasons) or ["涨停池接口缺失时不得生成情绪强弱结论。"],
            details={"mode": "a_share_characteristics", "admitted": False, "coverage_status": "insufficient", "source_ids": [], "as_of": context.as_of},
        )

    sealed_rate = float(context.sealed_limit_up_rate)
    failed_rate = float(context.failed_breakout_rate)
    high_board_count = sum(
        count for label, count in context.board_ladder.items() if label in set(config["high_board_labels"])
    )
    ...
    base_stage = (
        "封板强" if sealed_rate >= float(config["strong_seal_rate_pct"])
        else "封板弱" if sealed_rate < float(config["weak_seal_rate_pct"])
        else "封板分歧"
    )
    stage = base_stage if ladder_is_complete else f"{base_stage}（梯队待核验）"
    ...
    return SkillInsight(
        ...
        stage=stage,
        score=final_score,
        conclusion=f"已核验的封板与炸板证据显示{base_stage}；" + ("连板梯队缺失，未按零值计分。" if not ladder_is_complete else ""),
        strategy="把封板质量作为市场状态证据；梯队未核验时不推断接力高度。",
        evidence=[
            ...
            f"连板梯队 {ladder}" if ladder_is_complete else "连板梯队待核验，未按零值计分",
            f"数据时间：{context.as_of}",
        ],
        risks=[
            "涨跌停池口径不含部分特殊交易状态，不能直接代表全市场。",
            "一字板数量高也可能意味着可交易性差。",
        ...
        details={
            ...
            "admitted": True,
            "coverage_status": "complete" if ladder_is_complete else "partial",
