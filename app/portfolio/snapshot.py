...
def quote_advice(quote: RealtimeQuote | None) -> str:
    if quote is None or quote.data_status != "real_time" or quote.change_pct is None:
        return "行情暂不可用；不要依据旧价调整计划，先刷新或运行完整研究。"
    if quote.change_pct <= -5:
        return "当日跌幅显著：先核验公告、趋势和流动性，避免在风险未澄清前补仓。"
    if quote.change_pct <= -2:
        return "当日偏弱：检查是否跌破个人战法的失效条件，而不是只看成本价。"
    if quote.change_pct >= 5:
        return "当日波动显著：不把强势本身当信号，核验资金连续性与题材阶段。"
    if quote.change_pct >= 2:
        return "当日偏强：优先等趋势与成交额确认，避免追逐单日涨幅。"
    return "当日波动温和：可结合当前风格原型运行完整研究，再决定是否继续观察。"
